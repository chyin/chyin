...
function pieChart(question) {
	...
	var canvasAbsPo;                  // Absolute position of the canvas
	var centreX;                      // X-coordinate of centre of the canvas/chart
	var centreY;                      // Y-coordinate of centre of the canvas/chart
	var centreXX;                     // X-coordinate of centre of the canvas/chart in total page
	var centreYY;                     // Y-coordinate of centre of the canvas/chart in total page
	...
	*/
	...
	function init() {
		...
		canvasAbsPo = getElementAbsPos(canvas);
		canvasWidth = canvas.width;
		canvasHeight = canvas.height;
		centreX = canvasWidth / 2;
		centreY = canvasHeight / 2;
		centreXX = canvasAbsPo.left + centreX;
		centreYY = canvasAbsPo.top + centreY;
		...
		$('#chartData'+question+' td').each( function() {
			currentCell++;
			if ( currentCell % 3 != 1 ) {
				if ( currentCell % 3 === 2 ) {
					currentRow++;
		...
		drawChart();
		$('#chart'+question).click ( handleChartClick );
	}
	...
	*/
	...
	function handleChartClick ( clickEvent ) {
		...
		var xFromCentre = mouseX - centreXX;
	...
	*/
	...
	function startPullOut ( slice ) {
		...
		$('#chartData'+question+' td').removeClass('highlight');
		var codeCell = $('#chartData'+question+' td:eq(' + (slice*3) + ')');
		var labelCell = $('#chartData'+question+' td:eq(' + (slice*3+1) + ')');
		var valueCell = $('#chartData'+question+' td:eq(' + (slice*3+2) + ')');
		codeCell.addClass('highlight');
	...
	*/
	...
	function drawSlice ( context, slice ) {
		...
		if ( slice == currentPullOutSlice ) {
			...
			context.fillText( pullOutValuePrefix + parseInt(chartData[slice]['value']) + " (" + ( parseInt( chartData[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 20 );
